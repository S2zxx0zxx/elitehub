  ...
  return (
    <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
      {categories.map((cat, i) => (
        <div 
          key={i} 
          className="bg-surface-dark border border-white/5 rounded-2xl p-4 flex flex-col items-center justify-center text-center gap-2 hover:border-brand-yellow/30 transition-colors"
        >
          <cat.icon className="text-brand-yellow w-6 h-6" />
          <span className="text-elite-white text-xs font-bold">{cat.label}</span>
        </div>
      ))}
    </div>
  );
